...
function UILib.new(config)
    ...
    local function refreshTabPositions(activeName)
        local activeIdx = 1
        for i, t in ipairs(tabList) do if t.name == activeName then activeIdx = i break end end
    ...
    Window._tabBtns   = tabBtns
    Window._pages     = pages
    Window._TabBar    = TabBar
    Window._PageArea  = PageArea
    Window._switchTo  = switchTo
    Window._W         = W
    Window._SMALL     = SMALL
    Window._EXPAND    = EXPAND
    Window._ICON_S    = ICON_S
    Window._calcPos   = calcPositions

    function Window:AddTab(cfg)
        cfg = cfg or {}
        local name = cfg.Name or ("Tab"..tostring(#tabList+1))
        local sub  = cfg.Sub  or ""
        local icon = cfg.Icon or "rbxassetid://10709818534"
        ...
        pages[name] = pg
        table.insert(tabList, {name = name, sub = sub, icon = icon})
        ...
        local bg = ni("Frame", {
            Size = UDim2.new(0, SMALL, 1, 0), Position = UDim2.new(0, p.x, 0, 0),
        ...
        local img = ni("ImageLabel", {
            ...
            BackgroundTransparency = 1, Image = icon, ImageColor3 = Theme.dim,
        }, sq)
        ...
        function Tab:AddSection(scfg)
            ...
            local sIcon = scfg.Icon or icon
            local side  = scfg.Side or "right"
            ...
            elseif side == "full" then
                ...
                for _, child in ipairs(pg:GetChildren()) do
                    if child:IsA("Frame") then
                        ...
                        if xScale < 0.1 then leftCount  = leftCount  + 1
