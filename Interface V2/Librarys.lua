...
local function writeSave()
	...
	if not ok then
		warn("[EcoHub] writeSave error: " .. tostring(err))
	end
	return ok
end
...
local function ni(class, props, parent)
	...
	for k, v in pairs(props) do
		...
		if not ok then
			warn("[EcoHub] ni error setting " .. tostring(k) .. ": " .. tostring(err))
		end
	end
...
function EcohubLibrarys.new(config)
	...
	ni("UIStroke", {Color = Color3.fromRGB(30, 80, 140), Thickness = 1.2}, Main)
	...
	ni("Frame", {
		...
		BackgroundColor3 = Theme.AcrylicMain, BorderSizePixel = 0, ZIndex = 2,
	}, TopBar)
	...
	ni("ImageLabel", {
		...
		Image = config.Logo or "", ScaleType = Enum.ScaleType.Fit, ZIndex = 4,
	}, logoFrame)
	...
	ni("Frame", {
		Size = UDim2.new(0,1,0,34), Position = UDim2.new(0,0,0.5,-17),
		BackgroundColor3 = Theme.TitleBarLine, BorderSizePixel = 0, ZIndex = 4,
	}, UserBlock)

	local AvatarFrame = ni("Frame", {
		Size = UDim2.new(0,34,0,34), Position = UDim2.new(0,12,0.5,-17),
		BackgroundColor3 = Theme.elemBg, BorderSizePixel = 0, ZIndex = 4,
	}, UserBlock)
	...
	local TabBar = ni("Frame", {
		...
		BackgroundColor3 = Theme.tabbar, BorderSizePixel = 0,
		...
		function tab:AddSection(cfg2)
			...
			if title ~= "" then
				local titleBar = ni("Frame", {
					...
					BackgroundColor3 = Theme.section, BorderSizePixel = 0, ZIndex = 2,
				}, target)
				local txtX = 12
				if iconId2 ~= "" then
					ni("ImageLabel", {
						...
						ImageColor3 = Theme.accentHi, ZIndex = 3,
					}, titleBar)
					txtX = 30
				end
				ni("TextLabel", {
				...
				ni("Frame", {
					...
					BackgroundColor3 = Theme.dim, BorderSizePixel = 0, ZIndex = 3,
				}, titleBar)
			end
			...
			function sec:AddToggle(cfg3)
				...
				local trkW, trkH = 34, 18
				local track = ni("Frame", {
					Size = UDim2.new(0,trkW,0,trkH),
					Position = UDim2.new(1,-trkW-10,0.5,-trkH/2),
				...
				local knob = ni("Frame", {
					...
					BackgroundColor3 = Theme.text, BorderSizePixel = 0, ZIndex = 7,
				}, track)
				corner(knob, knobSz)
				ni("UIStroke", {Color=Color3.fromRGB(120,200,255), Thickness=0.5}, knob)
			...
			function sec:AddSlider(cfg3)
				...
				ni("TextLabel", {
					Size = UDim2.new(1,-56,0,16), Position = UDim2.new(0,12,0,6),
					BackgroundTransparency = 1, Text = label,
					TextColor3 = Theme.text, TextSize = 10,
					Font = Enum.Font.Gotham,
					TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 5,
				...
				local valLbl = ni("TextLabel", {
					Size = UDim2.new(0,46,0,16), Position = UDim2.new(1,-52,0,6),
					BackgroundTransparency = 1,
				...
				local railBg = ni("Frame", {
					...
					BackgroundColor3 = Theme.dim, BorderSizePixel = 0, ZIndex = 5,
				}, row)
				corner(railBg, 6)
				...
				local fill = ni("Frame", {
					...
					BackgroundColor3 = Theme.accentHi, BorderSizePixel = 0, ZIndex = 6,
				}, railBg)
				corner(fill, 6)
				...
				local dot = ni("Frame", {
					...
					BackgroundColor3 = Theme.text, BorderSizePixel = 0, ZIndex = 8,
				}, railBg)
				corner(dot, 14)
			...
			function sec:AddKeybind(cfg3)
				...
				if savedKeyName then
					...
					if ok and k and k ~= Enum.KeyCode.Unknown then
						...
						warn("[EcoHub] AddKeybind: saved key invalid '" .. tostring(savedKeyName) .. "', using default.")
					end
				end
			...
			function sec:AddDropdown(cfg3)
				...
				local function calcDropH()
					return math.min(#options, MAX_VISIBLE) * OPT_H + PADDING * 2
				end

				local dropCanvas = ni("Frame", {
					BackgroundTransparency = 1,
				...
				corner(dropBg, 8)
				ni("UIStroke", {Color = Color3.fromRGB(30, 80, 140), Thickness = 1}, dropBg)
				addTexture(dropBg, 0.88, 101)
				...
				accentGrad.Color = ColorSequence.new({
					ColorSequenceKeypoint.new(0,    Color3.fromRGB(14, 13, 20)),
				...
				local function recalcCanvasPos()
					...
					if posY + dropH > screenH - 10 then
						posY = absPos.Y - dropH - 4
					end

					dropCanvas.Position = UDim2.fromOffset(absPos.X, posY)
					dropCanvas.Size     = UDim2.fromOffset(absSize.X + 6, dropH)
					dropBg.Size         = UDim2.fromOffset(absSize.X + 6, dropH)
				end

				local function applySelected()
					for idx2, ob in pairs(optButtons) do
						local iS = options[idx2] == selected
						tw(ob.row, {BackgroundColor3 = iS and Theme.accentLo or Color3.fromRGB(18, 16, 26)}, 0.14)
				...
				local function closeDD()
					...
					tw(dropBg,    {Size = UDim2.fromOffset(dropBg.AbsoluteSize.X, 0)}, 0.18, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
					task.delay(0.20, function() dropCanvas.Visible = false end)
				end
				...
				local function buildOptionItem(i2, opt)
					...
					local optRow = ni("Frame", {
						...
						BackgroundColor3 = isSel and Theme.accentLo or Color3.fromRGB(18, 16, 26),
						BorderSizePixel  = 0,
						ZIndex           = 105,
						LayoutOrder      = i2,
					}, optHolder)
					...
					optRow.InputBegan:Connect(function(input)
					...
					optRow.MouseEnter:Connect(function()
						if opt ~= selected then
							tw(optRow, {BackgroundColor3 = Color3.fromRGB(28, 24, 40)}, 0.1)
							tw(optLbl, {TextColor3 = Theme.text},                       0.1)
						end
					end)
					optRow.MouseLeave:Connect(function()
						if opt ~= selected then
							tw(optRow, {BackgroundColor3 = Color3.fromRGB(18, 16, 26)}, 0.1)
				...
				for i2, opt in ipairs(options) do
					...
					if not ok then
						warn("[EcoHub] AddDropdown buildOptionItem error: " .. tostring(err))
					end
				end
				...
				local function openDD()
					...
					local targetH = calcDropH()
					dropBg.Size = UDim2.fromOffset(dropBg.AbsoluteSize.X, 0)
					...
					tw(dropBg,    {Size = UDim2.fromOffset(dropCanvas.AbsoluteSize.X, targetH)}, 0.24, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
				end
				...
				selBox:GetPropertyChangedSignal("AbsolutePosition"):Connect(function()
					if open then recalcCanvasPos() end
				end)
				...
				UserInputService.InputBegan:Connect(function(input)
					...
					local rAbsPos = row.AbsolutePosition
					local rAbsSize = row.AbsoluteSize
					local inDrop = mPos.X >= absPos.X and mPos.X <= absPos.X + absSize.X
						and mPos.Y >= absPos.Y and mPos.Y <= absPos.Y + absSize.Y
					local inRow  = mPos.X >= rAbsPos.X and mPos.X <= rAbsPos.X + rAbsSize.X
						and mPos.Y >= rAbsPos.Y and mPos.Y <= rAbsPos.Y + rAbsSize.Y
				...
				function el:SetOptions(newOpts)
					...
					for i2, opt in ipairs(options) do
						local ok, err = pcall(buildOptionItem, i2, opt)
						if not ok then
							warn("[EcoHub] SetOptions buildOptionItem error: " .. tostring(err))
						end
					end
				end
			...
			function sec:AddParagraph(cfg3)
				...
				local txtLbl = ni("TextLabel", {
					...
					Position = UDim2.new(0,12,0,title ~= "" and 20 or 6),
			...
			function sec:AddButton(cfg3)
				...
				ni("TextLabel", {
					Size = UDim2.new(1,-46,1,0), Position = UDim2.new(0,14,0,0),
					BackgroundTransparency = 1, Text = label,
					TextColor3 = Theme.text, TextSize = 10,
					Font = Enum.Font.GothamBold,
					TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 5,
				...
				btn.MouseButton1Click:Connect(function()
					...
					tw(arrowImg, {ImageColor3 = Theme.accentHi},       0.1)
					task.delay(0.15, function()
						tw(row,      {BackgroundColor3 = Theme.elemBg}, 0.15)
						tw(arrowImg, {ImageColor3 = Theme.muted},       0.15)
					end)
					callback()
				end)
				...
				btn.MouseEnter:Connect(function()
					...
					tw(arrowImg, {ImageColor3 = Theme.text},           0.12)
				end)
				btn.MouseLeave:Connect(function()
					tw(row,      {BackgroundColor3 = Theme.elemBg},  0.12)
					tw(arrowImg, {ImageColor3 = Theme.muted},        0.12)
				end)
			...
			function sec:AddColorPicker(cfg3)
				...
				local saveId   = cfg3.SaveId   or label
				local default  = cfg3.Default  or Color3.fromRGB(80, 180, 255)
				local callback = cfg3.Callback or function() end
				...
				if savedColor and type(savedColor) == "table" then
					local ok, c = pcall(function()
						return Color3.fromRGB(savedColor.r or 80, savedColor.g or 180, savedColor.b or 255)
					end)
					if ok then color = c else
						warn("[EcoHub] AddColorPicker: invalid saved color for '" .. saveId .. "', using default.")
					end
				end
				...
				local picker = ni("Frame", {
					...
					BackgroundColor3 = Color3.fromRGB(10,10,14), BorderSizePixel = 0,
				...
				local satFrame = ni("Frame", {
					Size = UDim2.new(1,-16,0,60), Position = UDim2.new(0,8,0,34),
					BackgroundColor3 = Color3.fromRGB(255,255,255),
					BorderSizePixel = 0, ZIndex = 22,
				}, picker)
				corner(satFrame, 6)
				...
				openBtn.MouseButton1Click:Connect(function()
					...
					if open then
						...
						wrapper.Size   = UDim2.new(1,0,0,ELEM_H+H_PICKER+6)
						tw(picker, {Size=UDim2.new(1,0,0,H_PICKER)}, 0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
					else
						tw(picker, {Size=UDim2.new(1,0,0,0)}, 0.2, Enum.EasingStyle.Back, Enum.EasingDirection.In)
